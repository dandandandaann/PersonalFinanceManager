...
class GoogleSheetsExpenseLogger
{
    ...
    public GoogleSheetsExpenseLogger(string credentialsJson, string spreadsheetId, IEnumerable<Category> categories, ILogger logger)
    {
        ...
        var credential = GoogleCredential.FromJsonParameters(credentialParameters).CreateScoped(_scopes);
    ...
    public async Task LogExpense(string description, string amount, string category)
    {
        ...
        if (!double.TryParse(amount.Replace(',', '.'), out var doubleAmount))
        ...
        var sheetId = GetSheetId(_spreadsheetId, sheetName);
        _logger.LogInformation($"Found sheetId '{sheetId}'.");
        ...
        _logger.LogInformation("Updating rows.");
