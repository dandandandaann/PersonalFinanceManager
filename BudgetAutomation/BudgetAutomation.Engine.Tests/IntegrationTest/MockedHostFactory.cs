...
public class MockedHostFactory<TProgram> : IAsyncLifetime where TProgram : class
{
    ...
    private readonly Dictionary<string, string?> _testConfiguration = new()
    {
        ...
        { $"{SpreadsheetManagerApiClientSettings.Configuration}:Url", "http://localhost:0000/api/" },
        { $"{SpreadsheetManagerApiClientSettings.Configuration}:Key", "TEST_EXPENSE_API_KEY" },
