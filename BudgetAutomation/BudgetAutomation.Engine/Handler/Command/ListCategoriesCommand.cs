    ...
    ISpreadsheetManagerApiClient spreadsheetManagerApiClient) : ICommand
{
    ...
    public static string StaticCommandName => "listcategories";
    ...
    public async Task<Message> HandleAsync(Message message, CancellationToken cancellationToken = default)
    {
        ...
        var response =
            await spreadsheetManagerApiClient.ListCategoriesAsync(UserManagerService.Configuration.SpreadsheetId, cancellationToken);
        ...
        var categoriesHtml = "<b>Categorias:</b>\n" +
        string.Join("", response.Categories.Select(c => $"<b>•</b> {c}\n"));
        return await sender.ReplyAsync(chat,
            categoriesHtml,
